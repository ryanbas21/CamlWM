(** Pure, focused zipper-of-zippers — camlwm's central data structure.
    ...
    The reference module's Haddock is the authoritative spec — every operation
    here should match the semantics described there. When in doubt, read the
    xmonad implementation.
    ...
    A [Stack] is itself a zipper: a focused element with stacks of items above
    and below. This is what gives O(1) access to "the focused window" while
    still letting you walk left/right cheaply. *)

(** {1 Types} *)

type window = int
(** X11 windows are 32-bit XIDs. We model them as plain ints; the FFI layer
    converts. Using a nominal type would be safer — feel free to promote this to
    [type window = private int] once the API stabilises. *)

type workspace_tag = string
(** Workspaces are addressed by string tag ("1", "2", "web", …) so the user's
    config can name them however they like. *)
...
(** A non-empty focused list. [up] is stored reversed so that [focus_up] /
    [focus_down] are O(1). This matches xmonad. *)
...
val empty :
  layouts:'l -> tags:workspace_tag list -> screens:screen_detail list -> 'l t
(** Build a StackSet with the given workspace tags. The first
    [List.length screens] tags are attached to screens (one each); the rest
    become hidden workspaces. All workspaces start empty. *)
...
(** Bring focus to [window], possibly switching workspaces. No-op if not a
    member. *)

(** {1 Modification} *)

val insert_up : window -> 'l t -> 'l t
(** Insert window above the focus on the current workspace and focus it. No-op
    if already a member anywhere. *)
