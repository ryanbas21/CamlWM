...
let stack_member w = function
  | None -> false
  | Some { focus; up; down } ->
    focus = w || List.mem w up || List.mem w down
...
let focus_master_stack s =
  ...
  | m :: rest_rev ->
    ...
    { focus = m;
      up = [];
      down = List.rev rest_rev @ (s.focus :: s.down) }
...
let make_workspace ~layout tag : 'l workspace =
  { tag; layout; stack = None }
...
let empty ~layouts ~tags ~screens =
  ...
  | _ ->
    ...
    | current :: visible ->
      { current; visible; hidden; floating = [] }

let current_tag t = t.current.workspace.tag

let peek t =
  match t.current.workspace.stack with
  | None -> None
  | Some s -> Some s.focus
...
let all_workspaces t =
  t.current.workspace
  :: List.map (fun s -> s.workspace) t.visible
  @ t.hidden
...
let find_tag w t =
  let rec go = function
    | [] -> None
    | ws :: rest ->
      if stack_member w ws.stack then Some ws.tag else go rest
  in
  go (all_workspaces t)

(* Lift a Stack→Stack operation to a Workspace→Workspace operation. *)
let map_stack f ws =
  { ws with stack = Option.map f ws.stack }
...
let delete w t =
  ...
  in
  let visible' =
    List.map (fun scr -> { scr with workspace = remove_from_ws scr.workspace })
      t.visible
  in
  let hidden' = List.map remove_from_ws t.hidden in
  let floating' = List.remove_assoc w t.floating in
  { current = current'; visible = visible'; hidden = hidden'; floating = floating' }
...
let take_workspace tag t =
  ...
  | _ ->
    match
      List.partition (fun scr -> scr.workspace.tag = tag) t.visible
    with
...
let view tag t =
  if tag = current_tag t then t
    ...
    | From_visible (scr, rest_visible) ->
      ...
      let new_current = { scr with screen_detail = t.current.screen_detail;
                                    screen_id = t.current.screen_id } in
      let old_current_as_visible =
        { t.current with screen_detail = scr.screen_detail;
...
let insert_into_workspace tag w t =
  ...
  let visible' =
    List.map (fun scr -> { scr with workspace = modify_ws scr.workspace })
      t.visible
  in
...
let focus_window w t =
  ...
  | Some tag ->
    let t' = view tag t in
    map_current_workspace
      (fun ws -> map_stack (focus_window_stack w) ws) t'
