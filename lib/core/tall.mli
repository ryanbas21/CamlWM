(** xmonad's default layout: one big "master" window on the left,
...
(** {2 The shape we're computing}
    ...
    - n windows (n ≥ 2) → master takes left half;
      the remaining n−1 split the right half evenly. *)

val do_layout :
  screen:Stack_set.screen_detail ->
  Stack_set.window list ->
  (Stack_set.window * Geometry.rect) list
(** First element of the input list is the master window; the rest are
    slaves in stacking order. Returns one entry per input window. *)
