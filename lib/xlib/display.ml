...
let root_window t =
  Unsigned.ULong.to_int (Ffi.x_root_window t.raw t.screen)
...
let select_root_wm_events t ~window =
  let mask =
    Int64.logor
      Ffi.Event_mask.substructure_redirect
...
let map_window t w =
  ignore (Ffi.x_map_window t.raw (Unsigned.ULong.of_int w))
...
let keysym_of_string s =
  Unsigned.ULong.to_int (Ffi.x_string_to_keysym s)
