...
let launcher = [ "sh"; "-c"; "/home/ryan/.config/rofi/launchers/type-1/launcher.sh" ]
let window_switcher = [ "rofi"; "-show"; "window" ]
let browser = [ "firefox" ]
let lock_screen = [ "loginctl"; "lock-session" ]
let polybar_launch = [ "sh"; "-c"; "/home/ryan/.config/polybar/launch_polybar.sh" ]
let setup_layout = [ "sh"; "-c"; "/home/ryan/.config/i3/setup_layout.sh" ]
let display_setup = [ "sh"; "-c"; "/home/ryan/.config/scripts/display-setup.sh" ]
let volume_up = [ "sh"; "-c"; "/home/ryan/.local/bin/osd-volume +10" ]
let volume_down = [ "sh"; "-c"; "/home/ryan/.local/bin/osd-volume -10" ]
let volume_mute =
  [ "sh"; "-c";
    "pactl set-sink-mute @DEFAULT_SINK@ toggle && /home/ryan/.local/bin/osd-volume +0" ]
let mic_mute = [ "pactl"; "set-source-mute"; "@DEFAULT_SOURCE@"; "toggle" ]
let brightness_up = [ "sh"; "-c"; "/home/ryan/.local/bin/osd-brightness +10%" ]
let brightness_down = [ "sh"; "-c"; "/home/ryan/.local/bin/osd-brightness -10%" ]
let screenshot_gui = [ "flameshot"; "gui" ]
let screenshot_full = [ "flameshot"; "full"; "-p"; "/home/ryan/Documents/screenshots" ]
...
let my_bindings =
  ...
  @ Config.with_mod m
    [ ("j", Focus_next);
      ("k", Focus_prev);
      ("m", Focus_direction Down);  (* focusMaster not exposed; use directional *)
    ]
  (* Window management — swap *)
  @ Config.with_mod m_s
    [ ("Return", Swap_master);
      ("j", Key_binding.Focus_next);  (* swapDown not bound as action; closest *)
...
let spawn_all cmds =
  List.iter (fun cmd ->
    ...
    | 0 -> (try Unix.execvp (List.hd cmd) (Array.of_list cmd)
            with _ -> exit 127)
    | _ -> ()) cmds
...
let () =
  ...
  Camlwm_wm.Wm.run
    { Config.default with
      border_width = 1;
      focused_color = 0xFF0000;      (* red *)
      unfocused_color = 0xDDDDDD;    (* light grey *)
      gap = 20;
